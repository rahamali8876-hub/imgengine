...
//  * resize_nearest()
//  *
//  * Nearest-neighbour resize into a slab block.
//  * Correct and deterministic. AVX2 bilinear is next sprint.
//  */
// static img_result_t resize_nearest(
//     const img_buffer_t *src,
//     img_buffer_t *dst,
...
//     /* Resize photo once — reuse for all N×M cells */
//     img_buffer_t scaled = {0};
//     img_result_t r = resize_nearest(photo, &scaled, pw, ph, pool);
//     if (r != IMG_SUCCESS)
//         return r;

//     /* Allocate cell metadata from scratch arena */
//     uint32_t total = job->cols * job->rows;
//     img_cell_t *cells = img_arena_alloc(arena, sizeof(img_cell_t) * total);
//     if (!cells)
//     {
...
//      * BLIT GRID
//      *
//      * Cell origin: start_x + col * (photo_w + gap)
//      *              start_y + row * (photo_h + gap)
//      *
//      * The gap creates white space between photos.
//      * start_x/start_y from canvas_init centers the grid.
//      */
//     for (uint32_t row = 0; row < job->rows; row++)
//     {
//         for (uint32_t col = 0; col < job->cols; col++)
